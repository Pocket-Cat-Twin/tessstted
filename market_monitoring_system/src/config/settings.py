...
class YandexOCRConfig:
    """Simplified configuration for Yandex OCR API (single API only)."""
    api_key: str
    api_url: str
    max_retries: int = 3
    timeout: int = 30
...
class SettingsManager:
    ...
    def _parse_yandex_ocr_config(self) -> None:
        """Parse simplified Yandex OCR configuration."""
    ...
    def _validate_config(self) -> None:
        ...
        if self.yandex_ocr:
            ...
            if self.yandex_ocr.max_retries < 0:
                errors.append("OCR max_retries must be non-negative")
            if self.yandex_ocr.max_image_size_mb <= 0:
