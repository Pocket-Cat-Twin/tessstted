...
import { 
  users, config, faqs, emailTemplates, orders, orderGoods, stories, 
  customers, customerAddresses, subscriptionFeatures, userSubscriptions,
  blogCategories, storyCategoryRelations, storyTags, storyTagRelations 
} from './schema';
  ...
  try {
    ...
    if (!adminUser) {
      const inserted = await db.insert(users).values({
        ...
        password: '$2b$12$LQv3c1yqBwlzKzn8.OdKqOGaV9UlwT0jqk1QBgm1eJ8GhGYf9XVCG', // password: admin123
        name: 'Администратор',
    ...
    if (!regularUser) {
      const inserted = await db.insert(users).values({
        ...
        password: '$2b$12$LQv3c1yqBwlzKzn8.OdKqOGaV9UlwT0jqk1QBgm1eJ8GhGYf9XVCG', // password: user123
        name: 'Анна',
      ...
      const subscriptionFeaturesData = [
        {
          ...
          description: 'Обычный подписчик (БЕСПЛАТНО) - редкие заказы, знакомство с сервисом',
        },
        {
          tier: 'group' as const,
          ...
          hasPersonalSupport: false,
          hasProductInspection: false,
          description: 'Групповая подписка (990₽/месяц) - накопительные заказы, совместные покупки',
        },
        {
          tier: 'elite' as const,
          ...
          description: 'Элитный подписчик (1990₽/месяц) - активные клиенты, ценящие скорость и сервис',
        },
        {
          ...
          hasPersonalSupport: true,
          hasProductInspection: true,
          description: 'Разовый VIP-доступ (890₽ на 7 дней) - срочные разовые заказы',
        },
      ];
    ...
    if (!existingFaq) {
      ...
      const faqData = [
      {
        ...
        answer: 'Для создания заказа нажмите кнопку "Создать новый заказ" на главной странице, заполните информацию о товарах и контактные данные.',
        order: 1,
      },
      {
        question: 'Какая комиссия за заказ?',
        answer: 'Стандартная комиссия составляет 10% от стоимости товара. Окончательная сумма рассчитывается автоматически.',
        order: 2,
      },
      {
        question: 'Как происходит оплата?',
        answer: 'После подтверждения заказа вам будут высланы реквизиты для оплаты. Оплату можно произвести банковской картой или переводом.',
        order: 3,
      },
      {
        question: 'Сколько времени занимает доставка?',
        answer: 'Доставка из Китая обычно занимает 2-4 недели в зависимости от способа доставки и таможенного оформления.',
        order: 4,
      },
      {
        question: 'Можно ли отследить заказ?',
        answer: 'Да, после отправки товара вы получите трек-номер для отслеживания посылки.',
        order: 5,
      },
      ];
    ...
    if (!existingTemplates) {
      ...
      const emailTemplatesData = [
      {
        ...
        `,
        textContent: 'Добро пожаловать, {{name}}! Для подтверждения email перейдите по ссылке: {{verificationUrl}}',
      ...
      },
      {
        ...
        `,
        textContent: 'Заказ #{{orderNumber}} создан. Сумма: {{totalAmount}} ₽. Ссылка: {{orderUrl}}',
    ...
    const storiesData = [
      {
        ...
        `,
        excerpt: 'История о том, как я открыла для себя мир Lolita и сделала свой первый заказ через YuYu Lolita Shopping.',
      ...
      },
      {
        ...
        `,
        excerpt: 'Подробный гид по основным стилям Lolita моды для тех, кто только начинает своё знакомство с этим направлением.',
      ...
      },
      {
        ...
        `,
        excerpt: 'Полезные советы по уходу за Lolita платьями, которые помогут сохранить их первозданный вид.',
      ...
      if (!existingCategories) {
        ...
        const insertedCategories = await db.insert(blogCategories).values(categoriesData).returning();
        ...
        const insertedTags = await db.insert(storyTags).values(tagsData).returning();
        console.log('✅ Story tags seeded');
        
        // Link stories with categories and tags
        const categoryRelationsData = [
          { storyId: insertedStories[0].id, categoryId: insertedCategories[0].id }, // Товары
          { storyId: insertedStories[1].id, categoryId: insertedCategories[3].id }, // Гайды
          { storyId: insertedStories[2].id, categoryId: insertedCategories[3].id }, // Гайды
        ];
