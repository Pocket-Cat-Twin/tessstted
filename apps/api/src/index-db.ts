...
import { db, config, eq, orders, orderGoods, stories, users, customers, customerAddresses, desc } from '@yuyu/db';
...
const app = new Elysia()
    ...
    try {
      ...
      result.forEach(item => {
        try {
          ...
          configObj[item.key] = item.type === 'number' ? parseFloat(item.value) 
            : item.type === 'boolean' ? item.value === 'true'
            : item.value.startsWith('{') || item.value.startsWith('[') ? JSON.parse(item.value)
            : item.value;
        } catch {
  ...
  .get('/api/v1/config/kurs', async () => {
    try {
      const result = await db.select().from(config).where(eq(config.key, 'current_kurs')).limit(1);
  ...
  .post('/api/v1/auth/login', async ({ body }) => {
    ...
    if ((email === 'admin@yuyulolita.com' && password === 'admin123') ||
      ...
      return {
        ...
        data: {
          ...
          user: {
            ...
            email: email,
            name: email === 'admin@yuyulolita.com' ? 'Администратор' : 'Анна Петрова',
  ...
  .get('/api/v1/customers/:id', async ({ params }) => {
    try {
      ...
      const addresses = await db.select().from(customerAddresses).where(eq(customerAddresses.customerId, params.id));
  ...
  .onStart(async () => {
    ...
    if (dbConnected) {
      ...
      try {
        ...
        console.log('ℹ️ Seeding skipped (data may already exist):', error?.message || error);
      }
    }
  })
