...
export const faqRoutes = new Elysia({ prefix: '/faq' })
  ...
  .get('/category/:slug', async ({ params: { slug }, query }) => {
    const limit = parseInt(query.limit) || 20;
    const search = query.search;
  ...
  .put('/admin/categories/:id', async ({ params: { id }, body }) => {
    ...
    const [updatedCategory] = await db.update(faqCategories)
      .set({
        ...body,
        slug,
        updatedAt: new Date(),
