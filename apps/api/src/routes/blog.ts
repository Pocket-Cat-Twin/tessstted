...
import { NotFoundError, ValidationError, DuplicateError } from '../middleware/error';
...
export const blogRoutes = new Elysia({ prefix: '/blog' })
  ...
  .get('/categories/:slug', async ({ params: { slug } }) => {
  ...
  .get('/categories/:slug/stories', async ({ params: { slug }, query }) => {
    ...
    const storiesData = await db
      ...
      .from(stories)
      .innerJoin(storyCategoryRelations, eq(stories.id, storyCategoryRelations.storyId))
      .where(and(
  ...
  // Get stories by tag
  .get('/tags/:slug/stories', async ({ params: { slug }, query }) => {
    const page = parseInt(query.page) || 1;
    const limit = parseInt(query.limit) || 10;
    const offset = (page - 1) * limit;
  ...
  .get('/admin/categories', async ({ query }) => {
    const page = parseInt(query.page) || 1;
  ...
  // Create category
  .post('/admin/categories', async ({ body, set }) => {
    const { name, description, color, metaTitle, metaDescription, order, isActive, showOnHomepage } = body;
  ...
  // Update category
  .put('/admin/categories/:id', async ({ params: { id }, body }) => {
    const existingCategory = await db.query.blogCategories.findFirst({
  ...
  // Delete category
  .delete('/admin/categories/:id', async ({ params: { id } }) => {
    const category = await db.query.blogCategories.findFirst({
      where: eq(blogCategories.id, id),
    });
  ...
  // Create tag
  .post('/admin/tags', async ({ body, set }) => {
    ...
    const [newTag] = await db.insert(storyTags).values({
      name,
      slug,
      color: color || '#6B7280',
      usageCount: 0,
  ...
  // Update tag
  .put('/admin/tags/:id', async ({ params: { id }, body }) => {
    const existingTag = await db.query.storyTags.findFirst({
    ...
    const [updatedTag] = await db.update(storyTags)
      .set({
        ...body,
        slug,
        updatedAt: new Date(),
  ...
  // Delete tag
  .delete('/admin/tags/:id', async ({ params: { id } }) => {
    const tag = await db.query.storyTags.findFirst({
      where: eq(storyTags.id, id),
    });
  ...
  .post('/admin/stories/:storyId/categories', async ({ params: { storyId }, body }) => {
    ...
    await db.delete(storyCategoryRelations)
  ...
  .post('/admin/stories/:storyId/tags', async ({ params: { storyId }, body }) => {
    const { tagIds } = body;
    ...
    await db.delete(storyTagRelations)
      .where(eq(storyTagRelations.storyId, storyId));

    // Update usage counts (decrease for removed tags)
    if (existingTagIds.length > 0) {
      await db.update(storyTags)
