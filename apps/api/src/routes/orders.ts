...
export const orderRoutes = new Elysia({ prefix: '/orders' })
  ...
  .get('/processing-info', async ({ query, store }) => {
    ...
    detail: {
      ...
      description: 'Get processing time, storage limits, and restrictions based on user subscription',
      tags: ['Orders'],
    },
  })
  ...
  .post('/calculate', async ({ body }) => {
    ...
    const calculation = await orderService.calculateDetailedOrder(body.goods, currentKurs);
    ...
    detail: {
      ...
      description: 'Calculate detailed pricing including commission breakdown by tier',
      tags: ['Orders'],
    },
  })

  // Validate order creation (checks subscription limits)
  .post('/validate', async ({ body, store }) => {
    const userId = store?.user?.id;
    ...
    detail: {
      ...
      description: 'Check if user can create order based on subscription limits',
      tags: ['Orders'],
    },
  })
  ...
  .get('/storage-usage', async ({ store }) => {
    ...
    detail: {
      ...
      description: 'Get current storage usage and limits based on subscription',
      tags: ['Orders'],
    },
  })

  // Remove auth requirement for order creation and lookup
  .use(async ({ store, set }, next) => {
    // Allow unauthenticated access but store user if available
    return next();
  })
  ...
  .post('/', async ({ body, set, store, orderNumber, rateLimit }) => {
    ...
    const validationResult = await orderService.validateOrderCreation(userId, orderData);
    ...
    const commissionCalculation = calculateTotalCommission(items, currentKurs);
    ...
    const [newOrder] = await db.insert(orders).values({
      ...
      subtotalYuan: commissionCalculation.totals.originalPriceYuan.toString(),
      totalCommission: commissionCalculation.totals.totalCommissionRuble.toString(),
      currentKurs: currentKurs.toString(),
      totalYuan: (commissionCalculation.totals.originalPriceYuan + commissionCalculation.totals.totalCommissionYuan).toString(),
      totalRuble: commissionCalculation.totals.totalFinalPriceRuble.toString(),
    ...
    const orderGoodsData = orderData.goods.map((good, index) => {
      ...
      return {
        ...
        totalYuan: (good.priceYuan * good.quantity + itemCalculation.commissionYuan * good.quantity).toString(),
    ...
    return {
      ...
      data: { 
        ...
        processingInfo: {
          ...
          processingTimeHours: validationResult.processingInfo.processingTimeHours,
          storageTimeHours: validationResult.processingInfo.storageTimeHours,
          priorityProcessing: validationResult.processingInfo.priorityProcessing,
          processingDeadline: orderService.getProcessingDeadline(validationResult.processingInfo.subscriptionTier),
          storageExpiration: orderService.getStorageExpiration(validationResult.processingInfo.subscriptionTier),
        },
        ...
        commissionBreakdown: {
          ...
          finalPrice: {
            yuan: commissionCalculation.totals.originalPriceYuan + commissionCalculation.totals.totalCommissionYuan,
  ...
  .get('/:id', async ({ params: { id } }) => {
    ...
    const commissionBreakdown = order.goods?.map(good => {
      ...
      return {
        ...
        totals: {
          ...
          totalCommissionYuan: itemCommission.commissionYuan * good.quantity,
          totalCommissionRuble: itemCommission.commissionRuble * good.quantity,
          finalYuan: (Number(good.priceYuan) + itemCommission.commissionYuan) * good.quantity,
    ...
    if (order.user) {
      const processingInfo = await orderService.getOrderProcessingInfo(order.userId!);
    ...
    return {
      ...
      data: { 
        ...
        totals: {
          ...
          totalCommissionYuan: commissionBreakdown.reduce((sum, item) => sum + item.totals.totalCommissionYuan, 0),
  ...
  .post('/:orderId/goods', async ({ params: { orderId }, body }) => {
    ...
    const totalQuantityCommission = itemCommission.commissionYuan * goodData.quantity;
    ...
    const [newGood] = await db.insert(orderGoods).values({
      ...
      commission: (itemCommission.commissionRuble * goodData.quantity).toString(),
    ...
    const newTotalCalculation = calculateTotalCommission(allItems, Number(order.currentKurs));

    // Update order totals
    await db.update(orders)
      .set({
        subtotalYuan: newTotalCalculation.totals.originalPriceYuan.toString(),
        totalCommission: newTotalCalculation.totals.totalCommissionRuble.toString(),
        totalYuan: (newTotalCalculation.totals.originalPriceYuan + newTotalCalculation.totals.totalCommissionYuan).toString(),
        totalRuble: newTotalCalculation.totals.totalFinalPriceRuble.toString(),
        updatedAt: new Date(),
      })
      .where(eq(orders.id, orderId));
  ...
  .put('/goods/:goodId', async ({ params: { goodId }, body }) => {
    ...
    if (updateData.priceYuan || updateData.quantity) {
      ...
      const totalQuantityCommission = itemCommission.commissionYuan * newQuantity;
  ...
  // Bulk status update
  .post('/bulk/status', async ({ body, user }) => {
  ...
  .post('/bulk/summary', async ({ body }) => {
    ...
    const summary = {
      ...
      totalValue: {
        yuan: ordersData.reduce((sum, order) => sum + Number(order.totalYuan), 0),
        ruble: ordersData.reduce((sum, order) => sum + Number(order.totalRuble), 0),
        commission: ordersData.reduce((sum, order) => sum + Number(order.totalCommission), 0),
      },
      statusBreakdown: ordersData.reduce((acc, order) => {
        acc[order.status] = (acc[order.status] || 0) + 1;
        return acc;
      }, {} as Record<string, number>),
      totalItems: ordersData.reduce((sum, order) => 
        sum + (order.goods?.reduce((goodSum, good) => goodSum + good.quantity, 0) || 0), 0
      ),
      dateRange: {
        earliest: new Date(Math.min(...ordersData.map(o => o.createdAt.getTime()))),
        latest: new Date(Math.max(...ordersData.map(o => o.createdAt.getTime()))),
      },
    };
