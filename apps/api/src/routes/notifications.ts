...
export const notificationRoutes = new Elysia({ prefix: '/notifications' })
  ...
  .get('/history', async ({ store, query }) => {
    const userId = store.user.id;
    const limit = query.limit || 50;
    
    const history = await notificationService.getUserNotificationHistory(userId, limit);
  ...
  .put('/preferences', async ({ store, body }) => {
    const userId = store.user.id;
  ...
  .post('/admin/process-expiring', async () => {
    ...
    detail: {
      ...
      description: 'Manually trigger processing of expiring subscription notifications',
      tags: ['Notifications'],
    },
  })

  // Get expiring subscriptions schedule
  .get('/admin/expiring-schedule', async () => {
    const expiringSubscriptions = await notificationService.getExpiringSubscriptions();
    ...
    detail: {
      ...
      description: 'Get list of subscriptions that need expiration notifications',
      tags: ['Notifications'],
    },
  })

  // Get notification statistics
  .get('/admin/stats', async ({ query }) => {
  ...
  .post('/admin/send-custom', async ({ body }) => {
    const results = [];
