...
export const webhookRoutes = new Elysia({ prefix: "/webhooks" })
  ...
  .post(
    "/receive/:provider",
    async ({ params: { provider }, body, headers, set }) => {
  ...
  .post(
    ...
    async ({ body, set }) => {
      ...
      const invalidEvents = events.filter(
        (event) => !validEvents.includes(event as WebhookEvent),
      );
...
async function processExternalWebhook(
  provider: string,
  event: string,
  payload: any,
) {
...
function getEventDescription(event: WebhookEvent): string {
  const descriptions: Record<WebhookEvent, string> = {
    ...
    [WebhookEvent.INVENTORY_OUT_OF_STOCK]:
      "Triggered when item is out of stock",
