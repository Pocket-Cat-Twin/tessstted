...
export const adminStatsRoutes = new Elysia({ prefix: '/admin/stats' })
  ...
  .get('/dashboard', async ({ query }) => {
    ...
    const [{ totalRevenue }] = await db
      .select({ totalRevenue: sql<number>`COALESCE(SUM(${orders.totalRuble}), 0)` })
      .from(orders)
      .where(eq(orders.status, 'completed'));

    // Recent revenue (last period)
    const [{ recentRevenue }] = await db
      .select({ recentRevenue: sql<number>`COALESCE(SUM(${orders.totalRuble}), 0)` })
      .from(orders)
      .where(and(
      ...
      data: {
        overview: {
          ...
          revenue: {
            ...
            average: totalOrders ? Math.round((totalRevenue / totalOrders) || 0) : 0,
          },
          subscriptions: {
            total: totalSubscriptions || 0,
            active: activeSubscriptions || 0,
            new: newSubscriptions || 0,
            activeRate: totalSubscriptions ? Math.round((activeSubscriptions / totalSubscriptions) * 100) : 0,
          },
          content: {
            stories: {
              total: totalStories || 0,
              published: publishedStories || 0,
              publishRate: totalStories ? Math.round((publishedStories / totalStories) * 100) : 0,
            },
            faqs: {
              total: totalFAQs || 0,
              active: activeFAQs || 0,
