...
export const userRoutes = new Elysia({ prefix: '/users' })
  ...
  // Change password
  .put('/profile/password', async ({ body, user }) => {
  ...
  // Get user's orders
  .get('/profile/orders', async ({ user, query }) => {
  ...
  // Update user
  .put('/:id', async ({ params: { id }, body, user: currentUser }) => {
  ...
  // Block user
  .post('/:id/block', async ({ params: { id }, body, user: currentUser }) => {
  ...
  // Unblock user
  .post('/:id/unblock', async ({ params: { id }, user: currentUser }) => {
    // Find user
    const existingUser = await db.query.users.findFirst({
      where: eq(users.id, id),
    });
  ...
  // Delete user
  .delete('/:id', async ({ params: { id }, user: currentUser }) => {
    // Find user
    const existingUser = await db.query.users.findFirst({
      where: eq(users.id, id),
    });
