...
import { NotFoundError, UnauthorizedError, ValidationError, DuplicateError } from '../middleware/error';
...
export const authV2Routes = new Elysia({ prefix: '/auth/v2' })
  ...
  .post('/register', async ({ body, jwt, cookie, set }) => {
    const { registrationMethod, email, phone, password, name, fullName } = body;
    ...
    if (registrationMethod === 'email' && email) {
      ...
      if (existingUser) {
        throw new DuplicateError('User with this phone number already exists');
      }
    }
    ...
    try {
      if (registrationMethod === 'email' && email) {
        ...
        if (!verificationResult.success) {
          console.error('Failed to create email verification:', verificationResult.error);
        }
      } else if (registrationMethod === 'phone' && phone) {
        const verificationResult = await verificationService.createVerification({
          userId: newUser.id,
        ...
        if (!verificationResult.success) {
          console.error('Failed to create phone verification:', verificationResult.error);
        }
      }
    } catch (error) {
    ...
    return {
      success: true,
      message: registrationMethod === 'email' 
  ...
  .post('/login', async ({ body, jwt, cookie, set }) => {
    ...
    const needsVerification = loginMethod === 'email' 
      ? !user.emailVerified 
      : !user.phoneVerified;
  ...
  .post('/verify', async ({ body, set }) => {
    ...
    const result = await verificationService.validateVerification(token, code);
  ...
  .post('/resend-verification', async ({ body, set }) => {
    ...
    const isAlreadyVerified = type === 'email' ? user.emailVerified : user.phoneVerified;
    ...
    try {
      const verificationResult = await verificationService.createVerification({
        userId: user.id,
        type: type === 'email' ? 'email_registration' : 'phone_registration',
      ...
      if (!verificationResult.success) {
        ...
        throw new Error(verificationResult.error || 'Failed to send verification');
      }
  ...
  .post('/forgot-password', async ({ body, set }) => {
    ...
    try {
      ...
      if (!verificationResult.success) {
        ...
        throw new Error(verificationResult.error || 'Failed to send reset code');
      }
