...
export const schedulerRoutes = new Elysia({ prefix: '/scheduler' })
  ...
  .post('/jobs/storage-expirations', async () => {
    ...
    detail: {
      ...
      description: 'Manually trigger storage expiration processing and notifications',
      tags: ['Scheduler'],
    },
  })

  // Manually trigger subscription renewals
  .post('/jobs/subscription-renewals', async () => {
    ...
    detail: {
      ...
      description: 'Manually trigger auto-renewal processing for subscriptions',
      tags: ['Scheduler'],
    },
  })

  // Manually trigger data cleanup
  .post('/jobs/cleanup', async () => {
    ...
    detail: {
      ...
      description: 'Manually trigger cleanup of expired verification codes and sessions',
      tags: ['Scheduler'],
    },
  })

  // Run any job by name
  .post('/jobs/:jobName/run', async ({ params: { jobName } }) => {
    try {
  ...
  .get('/jobs/:jobName/history', async ({ params: { jobName }, query }) => {
    ...
    const mockHistory = Array.from({ length: Math.min(limit, 5) }, (_, i) => ({
      ...
      message: Math.random() > 0.8 ? 'Job failed due to network error' : 'Job completed successfully',
  ...
  .get('/health', async () => {
    ...
    const healthScore = overdueJobs.length === 0 ? 100 : 
    ...
    return {
      ...
      data: {
        ...
        recommendations: overdueJobs.length > 0 ? [
