...
export const storyRoutes = new Elysia({ prefix: '/stories' })
  ...
  .post('/', async ({ body, user, set }) => {
    ...
    const [newStory] = await db.insert(stories).values({
      ...
      authorId: user.id,
      ...(storyData.status === StoryStatus.PUBLISHED && { publishedAt: new Date() }),
    }).returning();
  ...
  // Update story images
  .put('/:id/images', async ({ params: { id }, body }) => {
  ...
  .post('/:id/publish', async ({ params: { id }, body }) => {
    ...
    const publishedAt = publish && existingStory.status !== StoryStatus.PUBLISHED 
      ? new Date() 
      : !publish 
        ? null 
        : existingStory.publishedAt;
