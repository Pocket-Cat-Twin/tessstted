...
export const storageRoutes = new Elysia({ prefix: "/storage" })
  ...
  .get(
    ...
    },
    {
      detail: {
        ...
        description:
          "Get storage limits and features for all subscription tiers",
        tags: ["Storage"],
      },
    },
  )
  ...
  .get(
    ...
    },
    {
      detail: {
        ...
        description:
          "Get current storage usage, limits, and expiry information",
        tags: ["Storage"],
      },
    },
  )
  ...
  .get(
    ...
    },
    {
      query: t.Object({
        status: t.Optional(
          t.Union([t.Literal("approaching_expiry"), t.Literal("expired")]),
        ),
      }),
      detail: {
  ...
  .get(
    ...
    async ({ store }) => {
      ...
      if (storageInfo.itemsExpired.length > 0) {
        warnings.push(
          `У вас ${storageInfo.itemsExpired.length} заказов с истекшим сроком хранения`,
        );
      }

      if (storageInfo.itemsApproachingExpiry.length > 0) {
        warnings.push(
          `${storageInfo.itemsApproachingExpiry.length} заказов скоро истекут (до 3 дней)`,
        );
      }

      if (storageInfo.nextExpiryDate) {
        const daysUntilNext = Math.ceil(
          (storageInfo.nextExpiryDate.getTime() - Date.now()) /
            (24 * 60 * 60 * 1000),
        );
        if (daysUntilNext <= 7) {
    ...
    },
    {
      detail: {
        ...
        description:
          "Get current storage warnings and recommendations for user",
        tags: ["Storage"],
      },
    },
  )
  ...
  .post(
    ...
    },
    {
      detail: {
        ...
        description:
          "Process expiring storage items and generate notifications",
        tags: ["Storage"],
      },
    },
  )
  ...
  .post(
    "/admin/move-to-delivery/:orderId",
    async ({ params: { orderId }, store }) => {
  ...
  .post(
    "/admin/extend-storage/:orderId",
    async ({ params: { orderId }, body, store }) => {
      const adminUserId = store.user.id;
      const result = await storageService.extendStorage(
        orderId,
        body.additionalDays,
        adminUserId,
      );
  ...
  .get(
    ...
    async ({ query }) => {
      ...
      const expiringSoon = items.filter(
        (item) => item.daysUntilExpiry !== null && item.daysUntilExpiry <= days,
      );
      ...
      return {
        ...
        data: {
          ...
          daysThreshold: days,
          summary: {
            byTier: expiringSoon.reduce(
              (acc, item) => {
                acc[item.subscriptionTier] =
                  (acc[item.subscriptionTier] || 0) + 1;
                return acc;
              },
    ...
    },
    {
      ...
      detail: {
        ...
        description:
          "Get items approaching storage expiry within specified days",
        tags: ["Storage"],
      },
    },
  )
      ...
      for (const orderId of orderIds) {
        try {
          if (operation === "move_to_delivery") {
            ...
            await storageService.extendStorage(
              orderId,
