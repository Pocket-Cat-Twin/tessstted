...
export const profileRoutes = new Elysia({ prefix: '/profile' })
  ...
  .get('/', async ({ store }) => {
    ...
    detail: {
      ...
      description: 'Get complete user profile including subscription details and addresses',
      tags: ['Profile'],
    },
  })
  ...
  .put('/', async ({ body, store }) => {
    ...
    detail: {
      ...
      description: 'Update user profile information (name, contact details, avatar)',
      tags: ['Profile'],
    },
  })

  // Get user addresses
  .get('/addresses', async ({ store }) => {
    const userId = store.user.id;
    const profile = await userProfileService.getUserProfile(userId);
  ...
  // Add new address
  .post('/addresses', async ({ body, store }) => {
    const userId = store.user.id;
  ...
  // Update address
  .put('/addresses/:addressId', async ({ params, body, store }) => {
    const userId = store.user.id;
  ...
  // Delete address
  .delete('/addresses/:addressId', async ({ params, store }) => {
    const userId = store.user.id;
    const { addressId } = params;
    ...
    detail: {
      ...
      description: 'Delete a delivery address (cannot delete the only address)',
      tags: ['Profile'],
    },
  })

  // Get subscription details
  .get('/subscription', async ({ store }) => {
    const userId = store.user.id;
    const profile = await userProfileService.getUserProfile(userId);
  ...
  .get('/subscription/history', async ({ store }) => {
    const userId = store.user.id;
  ...
  .post('/permissions/check', async ({ body, store }) => {
    const userId = store.user.id;
    const { actions } = body;
    ...
    for (const action of actions) {
      permissions[action] = await userProfileService.canUserPerformAction(userId, action);
    }
    ...
    detail: {
      ...
      description: 'Check if user can perform specific actions based on subscription tier',
      tags: ['Profile'],
    },
  })

  // Get storage limits
  .get('/storage-limit', async ({ store }) => {
    const userId = store.user.id;
