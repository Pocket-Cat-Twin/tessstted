...
export const configRoutes = new Elysia({ prefix: '/config' })
  ...
  .get('/commission', async () => {
    ...
    return {
      ...
      data: { 
        settings,
        description: {
          commission_low_threshold: 'Price threshold for low commission rate (CNY)',
          commission_low_rate: 'Commission rate for items below low threshold (decimal)',
          commission_standard_threshold: 'Price threshold for standard commission rate (CNY)',
          commission_standard_rate: 'Commission rate for items in standard range (decimal)',
          commission_high_flat_fee: 'Flat fee for items above high threshold (CNY)',
        }
      },
    };
  }, {
  ...
  .put('/commission', async ({ body }) => {
    ...
    for (const [key, value] of Object.entries(settings)) {
      ...
      if (isNaN(numValue) || numValue < 0) {
        throw new ValidationError(`Commission setting ${key} must be a positive number`);
      }
  ...
  .post('/commission/calculate', async ({ body }) => {
    ...
    } else if (priceYuan >= settings.lowThreshold && priceYuan <= settings.standardThreshold) {
