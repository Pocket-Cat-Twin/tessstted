...
import { subscriptionMiddleware, requireSubscription, requireFeature } from '../middleware/subscription';
...
export const subscriptionRoutes = new Elysia({ prefix: '/subscriptions' })
  ...
  .get('/status', async ({ store }) => {
    ...
    detail: {
      ...
      description: 'Get current subscription tier, expiration, and available upgrades',
      tags: ['Subscriptions'],
    },
  })

  // Purchase or upgrade subscription
  .post('/purchase', async ({ body, store }) => {
    const userId = store.user.id;
    ...
    body: t.Object({
      tier: t.Union([t.Literal('group'), t.Literal('elite'), t.Literal('vip_temp')]),
  ...
  .post('/cancel', async ({ body, store }) => {
    ...
    return {
      success: true,
      message: 'Subscription auto-renewal cancelled. Your current subscription will remain active until expiration.',
    };
  }, {
    body: t.Object({
  ...
  .get('/context', async ({ subscription }) => {
    return {
      success: true,
      data: { subscription },
    };
  ...
  .get('/features/promotions', async ({ subscription }) => {
    return {
      success: true,
      data: {
  ...
  .get('/features/combine-orders', async () => {
    return {
      success: true,
  ...
  .get('/features/priority-support', async () => {
    return {
      success: true,
      message: 'Welcome to priority support! Your Elite subscription gives you access to dedicated assistance.',
    };
  }, {
    detail: {
  ...
  .post('/admin/process-renewals', async () => {
  ...
  .get('/admin/users/:userId', async ({ params: { userId } }) => {
    const subscriptionInfo = await subscriptionService.getUserSubscriptionInfo(userId);
  ...
  .post('/admin/grant', async ({ body }) => {
    const result = await subscriptionService.purchaseSubscription(body.userId, {
      tier: body.tier,
