      ...
      app
        ...
        .get('/status', async () => {
          ...
          detail: {
            ...
            description: 'Returns backup configuration, status, and recent backups',
          },
        })
        ...
        .post('/create/full', async ({ query }) => {
          ...
          detail: {
            ...
            description: 'Creates a full database backup including schema and data',
          },
        })

        // Create schema-only backup
        .post('/create/schema', async ({ query }) => {
          try {
            const compress = query.compress !== 'false';
          ...
          detail: {
            ...
            description: 'Creates a schema-only backup (table structures, no data)',
          },
        })

        // Create data-only backup
        .post('/create/data', async ({ query }) => {
          try {
            const compress = query.compress !== 'false';
        ...
        .post('/create/custom', async () => {
          ...
          detail: {
            ...
            description: 'Creates a PostgreSQL custom format backup (best for large databases)',
          },
        })
        ...
        .post('/cleanup', async () => {
          ...
          detail: {
            ...
            description: 'Removes old backup files according to retention policy',
          },
        })

        // Restore from backup (DANGEROUS!)
        .post('/restore/:fileName', async ({ params, body }) => {
          try {
            // Additional safety check
            if (!body || typeof body !== 'object' || !('confirm' in body) || body.confirm !== 'YES_I_WANT_TO_RESTORE') {
              return {
                success: false,
                error: 'RESTORE_CONFIRMATION_REQUIRED',
                message: 'This operation will overwrite the current database. Send { "confirm": "YES_I_WANT_TO_RESTORE" } to proceed.',
              };
            }
          ...
          detail: {
            ...
            description: 'Restores database from a backup file. This will overwrite the current database!',
          },
        })
  );
