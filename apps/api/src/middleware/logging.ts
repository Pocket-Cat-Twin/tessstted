...
import { createRequestLogger, logApiMetrics, logSecurityEvent } from '../services/logger';
...
export const loggingMiddleware = new Elysia({ name: 'logging' })
  .derive(({ request, set }) => {
    ...
    const requestLogger = createRequestLogger(requestId, method, path, userAgent);
  ...
  .onAfterHandle(({ response, requestLogger, startTime, request, requestIp }) => {
  ...
  .onError(({ error, code, requestLogger, startTime, request, requestIp }) => {
    ...
    if (path.includes('..') || path.includes('<script>') || path.includes('union select')) {
...
export const logRateLimitViolation = (ip: string, path: string, limit: number) => {
