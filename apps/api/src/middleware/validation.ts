...
function validateFileExtension(fileName: string, mimeType: string): string | null {
...
export function validateFile(file: File, validationType: FileValidationType): FileValidationResult {
  const errors: string[] = [];
...
export function createFileValidationMiddleware(validationType: FileValidationType) {
  return new Elysia({ name: `fileValidation-${validationType}` })
    .onBeforeHandle(({ body, set }) => {
      ...
      for (const file of files) {
        ...
        if (!result.isValid) {
          validationErrors.push(`File '${file.name}': ${result.errors.join(', ')}`);
        }
      }
...
export const uploadSecurityMiddleware = new Elysia({ name: 'uploadSecurity' })
  .onBeforeHandle(({ body, set, headers }) => {
    ...
    if (body && typeof body === 'object') {
      const hasFiles = Object.values(body).some(value => 
...
export const virusScanMiddleware = new Elysia({ name: 'virusScan' })
  .onBeforeHandle(({ body, set }) => {
