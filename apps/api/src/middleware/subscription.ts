...
export function requireSubscription(minTier: 'group' | 'elite' | 'vip_temp') {
  ...
  return new Elysia({ name: `require-${minTier}` })
    ...
    .derive(({ subscription }) => {
      const currentLevel = subscription.tier === 'free' ? 0 : tierLevels[subscription.tier] || 0;
 ...
 */
export function requireFeature(featureName: keyof typeof SUBSCRIPTION_TIERS.free.features) {
    ...
    .derive(({ subscription }) => {
      ...
      if (!hasFeature) {
        const featureMessages = {
          canParticipateInPromotions: 'Участие в акциях доступно только для подписчиков',
          canCombineOrders: 'Объединение заказов доступно для групповой подписки и выше',
          hasPriorityProcessing: 'Приоритетная обработка доступна для элитных подписчиков',
          hasPersonalSupport: 'Персональная поддержка доступна для элитных подписчиков',
          hasProductInspection: 'Проверка товаров доступна для элитных подписчиков',
        };

        const message = featureMessages[featureName] || `Feature ${featureName} not available`;
...
export function checkStorageLimit() {
  return new Elysia({ name: 'storage-limit' })
    ...
    .derive(async ({ subscription, store }) => {
      ...
      const storageExpiration = new Date(Date.now() - maxStorageDays * 24 * 60 * 60 * 1000);
      const expiredOrders = activeOrders.filter(order => order.createdAt <= storageExpiration);

      if (expiredOrders.length > 0) {
        throw new ForbiddenError(`${expiredOrders.length} orders have exceeded storage limit. Please arrange delivery.`);
      }
      ...
      return {
        storageInfo: {
          ...
          approaching: activeOrders.filter(order => {
            const daysInStorage = Math.floor((Date.now() - order.createdAt.getTime()) / (24 * 60 * 60 * 1000));
 ...
 */
export function subscriptionRateLimit(action: 'order_creation' | 'support_request') {
  return new Elysia({ name: `rate-limit-${action}` })
    ...
    .derive(async ({ subscription, store }) => {
      ...
      const windowStart = new Date(Date.now() - tierLimits.windowHours * 60 * 60 * 1000);
      ...
      return {
        rateLimit: {
          ...
          resetTime: new Date(Date.now() + tierLimits.windowHours * 60 * 60 * 1000),
        },
      };
    });
}
  ...
  return new Elysia({ name: 'subscription-warnings' })
    ...
    .derive(({ subscription }) => {
      ...
      if (subscription.tier === 'free') {
        warnings.push('Upgrade to Group subscription for faster processing and promotions access');
      }

      // Expiring subscription warnings
      if (subscription.daysRemaining !== null && subscription.daysRemaining <= 7) {
        if (subscription.daysRemaining <= 0) {
          warnings.push('Your subscription has expired. Upgrade to maintain premium features.');
        } else {
          warnings.push(`Your subscription expires in ${subscription.daysRemaining} days.`);
        }
      }

      // VIP temp specific warnings
      if (subscription.tier === 'vip_temp' && subscription.daysRemaining !== null) {
        warnings.push(`VIP access expires in ${subscription.daysRemaining} days. Consider upgrading to Elite for permanent benefits.`);
      }
    ...
    .derive(async ({ subscription, store }) => {
      ...
      if (tier === 'vip_temp' && userId) {
        ...
        if (activeSubscription) {
          ...
          if (ordersInPeriod.length >= 1) {
            throw new ForbiddenError('VIP временный доступ ограничен 1 заказом за период');
          }
        }
      }
      ...
      return {
        orderValidation: {
          ...
          storageTimeHours: subscription.features.maxStorageDays === -1 ? -1 : subscription.features.maxStorageDays * 24,
...
export function checkOrderValueAndRecommendUpgrade() {
  return new Elysia({ name: 'order-value-recommendations' })
    ...
    .derive(({ subscription }) => {
      ...
      if (subscription.tier === 'free') {
        recommendations.push('Для крупных заказов рекомендуем групповую или элитную подписку');
        recommendations.push('Групповая подписка: быстрая обработка, хранение до 3 месяцев');
        recommendations.push('Элитная подписка: приоритетная обработка, неограниченное хранение');
      } else if (subscription.tier === 'group') {
        recommendations.push('Элитная подписка: приоритетная обработка и персональная поддержка');
      }
      ...
      return {
        ...
        currentTierBenefits: {
          ...
          storage: subscription.features.maxStorageDays === -1 
            ? 'Неограниченное хранение' 
            : `${subscription.features.maxStorageDays} дней хранения`,
          promotions: subscription.features.canParticipateInPromotions ? 'Доступ к акциям' : 'Нет доступа к акциям',
          combineOrders: subscription.features.canCombineOrders ? 'Объединение заказов' : 'Нет объединения',
        },
      };
    });
}
...
export function orderSubscriptionMiddleware() {
  return new Elysia({ name: 'order-subscription-complete' })
    ...
    .derive(({ subscription, orderValidation, orderPriority, upgradeRecommendations, subscriptionWarnings }) => {
      return {
        orderContext: {
