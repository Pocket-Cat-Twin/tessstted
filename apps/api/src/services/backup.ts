...
const generateBackupFileName = (type: string, compress: boolean = true): string => {
...
const executePgDump = async (options: BackupOptions): Promise<BackupResult> => {
  ...
  const fileName = options.fileName || generateBackupFileName(options.type, options.compress);
  ...
  return new Promise((resolve, reject) => {
    ...
    const args = [
      ...
      '--username', BACKUP_CONFIG.database.username,
...
// Create full backup
export const createFullBackup = async (compress: boolean = true): Promise<BackupResult> => {
  await ensureBackupDirectory();
  return executePgDump({ type: 'full', compress });
};

// Create schema-only backup
export const createSchemaBackup = async (compress: boolean = true): Promise<BackupResult> => {
  await ensureBackupDirectory();
  return executePgDump({ type: 'schema', compress });
};

// Create data-only backup
export const createDataBackup = async (compress: boolean = true): Promise<BackupResult> => {
  await ensureBackupDirectory();
        ...
        .map(async (file) => {
          ...
          return {
            ...
            type: file.includes('-schema-') ? 'schema' : 
                  ...
                  file.includes('-custom-') ? 'custom' : 'full',
...
export const cleanupOldBackups = async (): Promise<{ deleted: string[]; kept: string[] }> => {
  ...
  try {
    ...
    if (deleted.length > 0) {
      logBusinessEvent(
        ...
        'system',
        'backup-service',
        { deletedCount: deleted.length, keptCount: kept.length }
      );
    }
...
// Restore from backup
export const restoreFromBackup = async (backupFileName: string): Promise<boolean> => {
  ...
  try {
    ...
    return new Promise((resolve, reject) => {
      const args = [
        ...
        '--verbose',
        '--dbname', BACKUP_CONFIG.database.database,
      ];
      ...
      if (backupFileName.endsWith('.dump')) {
        ...
        if (backupFileName.endsWith('.gz')) {
          // Compressed SQL file
          const gzipStream = createReadStream(backupPath).pipe(require('zlib').createGunzip());
      ...
      restoreProcess.on('close', (code) => {
        if (code === 0) {
          backupLogger.info('Database restore completed successfully', { backupFileName });
          logBusinessEvent(
    ...
    backupLogger.error('Failed to restore from backup', { error, backupFileName });
    return false;
  }
};
