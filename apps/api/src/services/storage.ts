import { db, orders, userSubscriptions, eq, and, gte, lt, desc, sql } from '@yuyu/db';
...
class StorageService {
   ...
   */
  private async getUserSubscriptionTier(userId: string): Promise<'free' | 'group' | 'elite' | 'vip_temp'> {
   ...
   */
  private calculateStorageExpiry(storageEntryDate: Date, tier: 'free' | 'group' | 'elite' | 'vip_temp'): Date | null {
    ...
    return new Date(storageEntryDate.getTime() + maxStorageDays * 24 * 60 * 60 * 1000);
  }
  ...
  async getStorageItems(filters?: {
    ...
    for (const order of ordersInStorage) {
      ...
      const storageExpiryDate = this.calculateStorageExpiry(storageEntryDate, tier);

      const now = new Date();
      const daysInStorage = Math.floor((now.getTime() - storageEntryDate.getTime()) / (24 * 60 * 60 * 1000));
      
      let daysUntilExpiry: number | null = null;
      let status: StorageItem['status'] = 'in_storage';

      if (storageExpiryDate) {
        daysUntilExpiry = Math.floor((storageExpiryDate.getTime() - now.getTime()) / (24 * 60 * 60 * 1000));
  ...
  async getUserStorageInfo(userId: string): Promise<UserStorageInfo> {
    ...
    const itemsApproachingExpiry = userItems.filter(item => item.status === 'approaching_expiry');
    const itemsExpired = userItems.filter(item => item.status === 'expired');

    // Calculate storage usage percentage
    let storageUsagePercent: number | null = null;
    if (maxStorageDays !== -1) {
      const itemsNearExpiry = userItems.filter(item => 
        item.daysUntilExpiry !== null && item.daysUntilExpiry <= 3
      );
      storageUsagePercent = userItems.length > 0 ? 
        (itemsNearExpiry.length / userItems.length) * 100 : 0;
    }
    ...
    const itemsWithExpiry = userItems
      ...
      .sort((a, b) => (a.storageExpiryDate!.getTime() - b.storageExpiryDate!.getTime()));
  ...
  async processStorageExpirations(): Promise<{
    ...
    for (const item of allItems) {
      ...
      if (item.status === 'approaching_expiry' && item.daysUntilExpiry !== null) {
   ...
   */
  async moveToDelivery(orderId: string, adminUserId: string): Promise<{ success: boolean; message: string }> {
    const order = await db.query.orders.findFirst({
   ...
   */
  async extendStorage(orderId: string, additionalDays: number, adminUserId: string): Promise<{ success: boolean; message: string }> {
    const order = await db.query.orders.findFirst({
      where: eq(orders.id, orderId),
    });
