...
export const cleanupVerificationTokens = async (): Promise<number> => {
  ...
  cutoffDate.setDate(
    cutoffDate.getDate() - RETENTION_CONFIG.verificationTokens.expiredDays,
  );

  const completedCutoffDate = new Date();
  completedCutoffDate.setDate(
    completedCutoffDate.getDate() -
      RETENTION_CONFIG.verificationTokens.completedDays,
  );
  ...
  try {
    ...
    const result = await db.delete(verificationTokens).where(
      or(
...
export const cleanupSmsLogs = async (): Promise<number> => {
  const cutoffDate = new Date();
  cutoffDate.setDate(
    cutoffDate.getDate() - RETENTION_CONFIG.smsLogs.retentionDays,
  );
  ...
  try {
    ...
    if (deletedCount > 0) {
      logBusinessEvent("sms_logs_cleanup", "system", "cleanup-service", {
        deletedCount,
      });
    }
...
export const cleanupEmailLogs = async (): Promise<number> => {
  const cutoffDate = new Date();
  cutoffDate.setDate(
    cutoffDate.getDate() - RETENTION_CONFIG.emailLogs.retentionDays,
  );
  ...
  try {
    ...
    if (deletedCount > 0) {
      logBusinessEvent("email_logs_cleanup", "system", "cleanup-service", {
        deletedCount,
      });
    }
...
export const cleanupRateLimitEntries = async (): Promise<number> => {
  const cutoffDate = new Date();
  cutoffDate.setDate(
    cutoffDate.getDate() - RETENTION_CONFIG.rateLimitEntries.retentionDays,
  );
  ...
  try {
    ...
    if (deletedCount > 0) {
      logBusinessEvent("rate_limit_cleanup", "system", "cleanup-service", {
        deletedCount,
      });
    }
...
export const runFullCleanup = async (): Promise<{
  ...
  try {
    ...
    const [tokensDeleted, smsDeleted, emailDeleted, rateLimitDeleted] =
      await Promise.all([
    ...
    results.totalDeleted =
      tokensDeleted + smsDeleted + emailDeleted + rateLimitDeleted;
    ...
    logBusinessEvent("full_cleanup_completed", "system", "cleanup-service", {
      results,
      duration,
    });
...
export const getCleanupStatistics = async () => {
  ...
  cutoffDates.verificationTokensExpired.setDate(
    cutoffDates.verificationTokensExpired.getDate() -
      RETENTION_CONFIG.verificationTokens.expiredDays,
  );
  cutoffDates.verificationTokensCompleted.setDate(
    cutoffDates.verificationTokensCompleted.getDate() -
      RETENTION_CONFIG.verificationTokens.completedDays,
  );
  ...
  cutoffDates.rateLimitEntries.setDate(
    cutoffDates.rateLimitEntries.getDate() -
      RETENTION_CONFIG.rateLimitEntries.retentionDays,
  );
          ...
          or(
            and(
              lt(
                ...
                cutoffDates.verificationTokensExpired,
              ),
              eq(verificationTokens.status, "expired"),
            ),
            and(
              lt(
                ...
                cutoffDates.verificationTokensCompleted,
              ),
              or(
                eq(verificationTokens.status, "verified"),
                eq(verificationTokens.status, "failed"),
